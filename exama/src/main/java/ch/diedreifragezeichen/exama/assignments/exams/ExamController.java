...
public class ExamController {
    ...
    // public ModelAndView showSemesterView(@RequestParam(name = "selectedSemester")
    ...
    // semester.getStartDate().with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
    // LocalDate monday = semesterStart;
    // LocalDate semesterEnd =
    // semester.getEndDate().with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY));
    // // count all the exams
    ...
    // while (monday.isBefore(semesterEnd)) {
    // List<Exam> examsByWeek = examRepo.findAllByDueDateBetween(monday,
    // monday.with(DayOfWeek.SUNDAY));
    ...
    // long numOfDays =
    // ChronoUnit.DAYS.between(holiday.getStartDate().with(TemporalAdjusters.nextOrSame(DayOfWeek.MONDAY)),
    // holiday.getEndDate());
    // List<LocalDate> allDays = Stream.iterate(holiday.getStartDate(), date ->
    // date.plusDays(1))
